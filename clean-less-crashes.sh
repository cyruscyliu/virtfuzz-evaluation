...
cd ../qemu/build-clean-less-crashes-$VERSION
if [ $VERSION == '6' ]; then
    CC=clang CXX=clang++ ../configure --enable-fuzzing --enable-debug \
        --disable-werror \
        --extra-cflags="-DVIRTFUZZ_LESS_CRASHES" \
    ...
    CC=clang CXX=clang++ ../configure --enable-debug \
        --disable-werror \
        --target-list="i386-softmmu arm-softmmu aarch64-softmmu"
    make CONFIG_FUZZ=y CFLAGS="-fsanitize=fuzzer -g -DVIRTFUZZ_LESS_CRASHES" -j$(nproc) \
